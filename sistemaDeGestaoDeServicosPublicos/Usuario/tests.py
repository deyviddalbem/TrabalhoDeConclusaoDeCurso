...
# class teste_login(unittest.TestCase):
    ...
    # def teste_login_incorreto(self):
        ...
        # Relatório====>>> bugs encontrados==>> quando o usuário digita a senha incorreta, não mostra nenhuma mensagem!
...
class teste_cadastro_Endereco_correto(unittest.TestCase):

    def setUp(self):
        self.driver = webdriver.Firefox()
        self.driver.get('http://127.0.0.1:8000/Usuario/cadastroEndereco/')

    # testando se a app está inserindo dados corretos para efetuar o login

    def teste_endereco_correto(self):
        self.usuario = self.driver.find_element_by_name("cep")
        self.usuario.send_keys("76983480")
        self.usuario = self.driver.find_element_by_name("enderecoNumero")
        self.usuario.send_keys("ay890")
        self.usuario = self.driver.find_element_by_name("complemento")
        self.usuario.send_keys("ap")
        self.usuario = self.driver.find_element_by_name("observacao")
        self.usuario.send_keys("casa de esquina")
        # Relatório ==>>> se os dados estiverem corretos, serão salvos no banco,, FUNCIONOU!!

        self.usuario.submit()

    def teste_endereco_incorreto(self):
        self.usuario = self.driver.find_element_by_name("cep")
        self.usuario.send_keys("76967669")
        # Relatório ==>>> se o CEP estiver errado, não existir ou não ser preenchido o campo, os dados não podem ser salvos, FUNCIONOU!
